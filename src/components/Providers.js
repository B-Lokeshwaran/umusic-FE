...
function Providers() {
    ...
    useEffect(() => {
        const value = async () => {
            const response = await axios.post("https://umusic-b.herokuapp.com/providers/getbyname", data)
            console.log(response.data);
    ...
    useEffect(() => {
        setTimeout(() => {
            axios.get(`https://umusic-b.herokuapp.com/providers/${pagination.limit}/${pagination.skip}`)
                .then(res => {
            ...
            <Row>
                ...
                <Col sm={9} style={{ 'marginTop': '-790px', 'marginLeft': '-10px' }}>
                    <div className="providers" style={{ 'display': 'flex', 'width': '970px' }}>
                        <h2 style={{ 'fontWeight': 'normal' }}><IoIosLock color="#B6B6B6" fontSize="40px" style={{ 'marginRight': '20px', 'marginTop': '-7px' }} />Providers<Button variant="info" className='appbtn'><Link to="/providers/addpro">Add New</Link></Button></h2>
                                    ...
                                    return (
                                        <tbody>
                                            <tr>
                                                ...
                                                <td><Link to={`/providers/viewall/${data._id}`} state={data}>{data._id}</Link></td>
                                                <td><Link to={`/providers/viewall/${data._id}`} state={data}>{data.secret}</Link></td>
                                                <td><Link to='/providers/editpro' state={data}><Button variant="outline-secondary">Edit</Button></Link></td>
                                            </tr>
                                        </tbody>
                                    )
                                }
