...
export default function AddPro() {
    const [data, setData] = useState({name: "", secret: ""});
    ...
    const providerIDHandler = (event) => {
        setData((previousvalue) => {
            return {...previousvalue, secret: event.target.value};
        });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        let data1 = [data];
        console.log(data1);
        axios.post('https://umusic-b.herokuapp.com/providers', data1);
    }
    return (
        <div>
            <Row>
                ...
                <Col sm={9} style={{ 'marginTop': '-790px', 'marginLeft': '-11px' }}>
                    ...
                    <div className="add-btn">
                        <div className="add-btn1">
                            <Button variant="outline-secondary"><Link to="/providers">Cancel</Link></Button>
                        </div>
