...
export default function Shopify() {
    ...
    useEffect(() => {
        const value = async () => {
            const response = await axios.post("https://umusic-b.herokuapp.com/shopifystores/getbyname", data)
    ...
    useEffect(() => {
        setTimeout(() => {
            axios.get(`https://umusic-b.herokuapp.com/shopifystores/${pagination.limit}/${pagination.skip}`)
                .then(res => {
                ...
                <Row>
                    ...
                    <Col sm={9} style={{ 'marginTop': '-800px', 'marginLeft': '-48px' }}>
                        <div className="shopify" style={{ 'marginTop': '10px', 'display': 'flex' }}>
                            <h2 style={{ 'fontWeight': 'normal' }}><FaShopify color="#B6B6B6" fontSize="40px" style={{ 'marginRight': '20px' }} />Shopify Stores<Button variant="info" className='appbtn'><Nav.Link as={Link} to='/shopifystores/addshop'>Add New</Nav.Link></Button></h2>
                                ...
                                return (
                                    <tbody>
                                        <tr>
                                            ...
                                            <td><Link to={`/shopifystores/viewall/${data._id}`} state={data}>{data._id}</Link></td>
                                            <td><Link to={`/shopifystores/viewall/${data._id}`} state={data}>{data.secret}</Link></td>
                                            <td><Link to='/shopifystores/editshop' state={data}><Button variant="outline-secondary">Edit</Button></Link></td>
                                        </tr>
                                    </tbody>
                                )
                            }
